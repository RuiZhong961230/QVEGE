...
def Maturity(func):
    ...
    for i in range(PopSize):
        for j in range(SeedNum):
            ...
            seedFit[i*SeedNum + j] = func.evaluate(seedIndi[i*SeedNum + j])
            Times_table[index] += 1
...
def RunVEGE(func):
    ...
    with open('./QVEGE_Data/' + SuiteName + '/F{}_{}D.csv'.format(FuncNum, DimSize), "ab") as f:
        np.savetxt(f, All_Trial_Best, delimiter=",")


def main_CEC2020(Dim):
    ...
    for i in range(len(CEC2020Funcs)):
        FuncNum = i+1
        RunVEGE(CEC2020Funcs[i])


if __name__ == "__main__":
    if os.path.exists('./QVEGE_Data/CEC2020') == False:
        os.makedirs('./QVEGE_Data/CEC2020')
    Dims = [10, 30, 50, 100]
    for Dim in Dims:
        main_CEC2020(Dim)
